...
```
check_s3_input          S3에 당일 원천 데이터 존재 확인
  → start_cluster       EC2 인스턴스 시작 (Master + Worker ×4)
  → start_spark         workers 파일 갱신 + Spark 클러스터 기동
  → download_code       S3 → Master /tmp/spark-job/ 코드 배포
  → install_deps        Master + Worker 전체에 pyarrow 병렬 설치
  → run_stage1          Spark: 이상 탐지 (Z-score + impact_score)
  → check_s3_stage1     Stage1 출력 존재 확인
  → run_stage2          Spark: 공간 클러스터링 (GPS → 세그먼트 매핑)
  → check_s3_stage2     Stage2 출력 존재 확인
  → load_to_rdb         S3 Parquet → PostgreSQL UPSERT
  → refresh_views       Materialized View 3개 CONCURRENTLY 갱신
  → send_email_report   일일 리포트 발송
  → stop_spark          Spark 클러스터 종료 (always run)
  → stop_cluster        EC2 인스턴스 중지 (always run)
```
...
airflow_service/
├── dags/
│   ├── pothole_pipeline_dag.py   # DAG 정의 (14개 태스크)
